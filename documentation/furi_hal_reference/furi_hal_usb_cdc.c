...
void tud_cdc_rx_cb(uint8_t itf) {
    ...
    if(callbacks[itf] && callbacks[itf]->rx_ep_callback)
...
void tud_cdc_line_state_cb(uint8_t instance, bool dtr, bool rts) {
    ...
    if(callbacks[instance] && callbacks[instance]->ctrl_line_callback)
...
void tud_mount_cb(void) {
    ...
    for(uint8_t i = 0; i < IF_NUM_MAX; i++)
        if(callbacks[i] && callbacks[i]->state_callback)
...
void tud_cdc_tx_complete_cb(uint8_t itf) {
    furi_check(itf < IF_NUM_MAX);
    if(callbacks[itf] && callbacks[itf]->tx_ep_callback)
...
void furi_hal_cdc_set_callbacks(uint8_t if_num, CdcCallbacks* cb, void* context) {
    ...
    if(callbacks[if_num] && callbacks[if_num]->state_callback && connected)
        callbacks[if_num]->state_callback(cb_ctx[if_num], 0);
    callbacks[if_num] = cb;
    cb_ctx[if_num] = context;
    if(callbacks[if_num]) {
        if(callbacks[if_num]->state_callback && connected)
            callbacks[if_num]->state_callback(cb_ctx[if_num], 1);
        if(callbacks[if_num]->ctrl_line_callback)
...
void furi_hal_cdc_send(uint8_t if_num, uint8_t* buf, uint16_t len) {
    ...
    if(status == FuriStatusErrorTimeout) return;
...
int32_t furi_hal_cdc_receive(uint8_t if_num, uint8_t* buf, uint16_t length) {
    ...
    if(tud_ready() && tud_cdc_n_available(if_num)) {
        FuriStatus status = furi_mutex_acquire(furi_hal_usb_cdc_mutex, FURI_HAL_USB_CDC_DEADLOCK_TIMEOUT_MS);
        if(status == FuriStatusErrorTimeout) return PICO_ERROR_NO_DATA;
        furi_check(status == FuriStatusOk);
        if(tud_ready() && tud_cdc_n_available(if_num)) {
