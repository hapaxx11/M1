...
static const FuriHalSpiResources furi_hal_spi_resources[FuriHalSpiIdMax] = {
    {
        ...
        .gpio = {&gpio_display_scl, &gpio_display_sda, NULL, &gpio_display_cs},
    },
    {
        .periph = spi1,
        .alt_fn = GpioAltFn1Spi,
        .gpio = {&gpio_cpu_spi_sck, &gpio_cpu_spi_mosi, NULL, &gpio_cpu_spi_cs},
    },
};

static FuriHalSpi* furi_hal_spi[FuriHalSpiIdMax];

void furi_hal_spi_init(FuriHalSpiHandle* handle, uint32_t baud_rate, FuriHalSpiTransferMode transfer_mode, FuriHalSpiTransferBitOrder bit_order, FuriHalSpiMode mode) {
    furi_check(handle);
    ...
    if(mode == FuriHalSpiModeSlave) spi_set_slave(periph, true);

    spi_order_t order = (bit_order == FuriHalSpiTransferBitOrderMsbFirst) ? SPI_MSB_FIRST : SPI_LSB_FIRST;
    spi_cpol_t cpol;
    spi_cpha_t cpha;
    switch(transfer_mode) {
    case FuriHalSpiTransferMode0: cpol = SPI_CPOL_0; cpha = SPI_CPHA_0; break;
    case FuriHalSpiTransferMode1: cpol = SPI_CPOL_0; cpha = SPI_CPHA_1; break;
    case FuriHalSpiTransferMode2: cpol = SPI_CPOL_1; cpha = SPI_CPHA_0; break;
    case FuriHalSpiTransferMode3: cpol = SPI_CPOL_1; cpha = SPI_CPHA_1; break;
    default: furi_crash("Invalid SPI transfer mode"); break;
    }
...
void furi_hal_spi_tx_blocking(FuriHalSpiHandle* handle, const uint8_t* tx_buffer, size_t size) {
    ...
    if(dma_channel_is_busy(spi->dma_tx_channel)) panic("TX completed");
