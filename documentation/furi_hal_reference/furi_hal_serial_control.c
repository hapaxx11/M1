...
static void furi_hal_serial_control_log_set_handle(FuriHalSerialHandle* handle) {
    ...
    if(handle) {
        ...
        furi_hal_serial_init(furi_hal_serial_control->log_serial, furi_hal_serial_control->log_config_serial_baud_rate);
...
static bool furi_hal_serial_control_handler_stop(void* input, void* output) {
    UNUSED(input); UNUSED(output); return false;
}
...
static bool furi_hal_serial_control_handler_logging(void* input, void* output) {
    ...
    if(furi_hal_serial_control->log_config_serial_id < FuriHalSerialIdMax) {
        if(!furi_hal_serial_control->handles[furi_hal_serial_control->log_config_serial_id].in_use)
            handle = &furi_hal_serial_control->handles[furi_hal_serial_control->log_config_serial_id];
    }
    furi_hal_serial_control_log_set_handle(handle);
    return true;
}
...
static int32_t furi_hal_serial_control_thread(void* args) {
    ...
    while(should_continue || furi_message_queue_get_count(furi_hal_serial_control->queue) > 0) {
        ...
        FuriStatus status = furi_message_queue_get(furi_hal_serial_control->queue, &message, FuriWaitForever);
        furi_check(status == FuriStatusOk);
        furi_check(message.type < COUNT_OF(furi_hal_serial_control_handlers));
        should_continue = furi_hal_serial_control_handlers[message.type](message.input, message.output);
        if(message.api_lock != NULL) api_lock_unlock(message.api_lock);
    }
    return 0;
}
...
void furi_hal_serial_control_init(void) {
    ...
    furi_hal_serial_control->queue = furi_message_queue_alloc(8, sizeof(FuriHalSerialControlMessage));
    furi_hal_serial_control->thread = furi_thread_alloc_service("SerialControlDriver", 512, furi_hal_serial_control_thread, NULL);
...
void furi_hal_serial_control_release(FuriHalSerialHandle* handle) {
    furi_check(furi_hal_serial_control); furi_check(handle);
    FuriHalSerialControlMessage message;
...
void furi_hal_serial_control_set_logging_config(FuriHalSerialId serial_id, uint32_t baud_rate) {
    ...
    furi_check(baud_rate >= 600 && baud_rate <= 7800000);
    if(!furi_hal_serial_control) return;
    furi_check(furi_hal_serial_is_baud_rate_supported(&furi_hal_serial_control->handles[serial_id], baud_rate));
    FuriHalSerialControlMessageInputLogging message_input = {.id = serial_id, .baud_rate = baud_rate};
    FuriHalSerialControlMessage message;
