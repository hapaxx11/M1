...
void furi_hal_gpio_init_ex(const GpioPin* gpio, const GpioMode mode, const GpioPull pull, const GpioSpeed speed, const GpioAltFn alt_fn) {
    ...
    switch(speed) {
    case GpioSpeedLow:  gpio_set_slew_rate(gpio->pin, GPIO_SLEW_RATE_SLOW); break;
    case GpioSpeedFast: gpio_set_slew_rate(gpio->pin, GPIO_SLEW_RATE_FAST); break;
    default: furi_crash("Incorrect GpioSpeed"); break;
    }
    switch(pull) {
    case GpioPullNo:   gpio_set_pulls(gpio->pin, false, false); break;
    case GpioPullUp:   gpio_set_pulls(gpio->pin, true,  false); break;
    case GpioPullDown: gpio_set_pulls(gpio->pin, false, true);  break;
    default: furi_crash("Incorrect GpioPull"); break;
    }
    switch(mode) {
    case GpioModeInput:
    ...
    case GpioModeAnalog:
        ...
        gpio_set_input_hysteresis_enabled(gpio->pin, false);
        break;
    default: furi_crash("Incorrect GpioMode"); break;
    }
    FURI_CRITICAL_EXIT();
}
...
void furi_hal_gpio_enable_int_callback(const GpioPin* gpio) {
    ...
    gpio_set_irq_enabled(gpio->pin,
        ...
        (gpio_interrupt[gpio->pin].condition == GpioConditionRiseFall) ? GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL : 0,
        true);
    FURI_CRITICAL_EXIT();
}
