...
| `Esp32_serial_flasher/` | `include/esp_loader.h` | `esp_loader_bin_segment_t.data` is `const uint8_t *` (was `uint8_t *`) | Lets the auto-generated `esp_stub[]` payloads bind to const compound literals so they live in flash (.rodata), not RAM (.data). | Hapax (issue #747) |
| `Esp32_serial_flasher/` | `src/esp_stubs.c` | Every stub payload is `.data = (const uint8_t[]){ ... }` (auto-generated file — re-apply `const` on regeneration) | Keeps the ~91 KB flasher-stub table in flash instead of RAM (freed ~91 KB, RAM 99.98% → 85.80%). Guarded by `tests/test_esp_stub_rodata.c`. | Hapax (issue #747) |
| `Esp32_serial_flasher/` | `src/protocol_uart.c`, `app/common/app_common.c` | `data_pos` reader pointers changed to `const uint8_t *` | Match the const `esp_loader_bin_segment_t.data` field; stub/segment data is transmit-only (never written). | Hapax (issue #747) |
