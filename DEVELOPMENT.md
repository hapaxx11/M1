...
right edge as a position indicator (3px-wide track at `M1_MENU_SCROLLBAR_X` = 124).
...
Draw selection highlights starting at x = `M1_MENU_HIGHLIGHT_X` (1) — a
1px inset from the left edge — with width `M1_MENU_TEXT_W` (122px), which
is derived as `M1_MENU_SCROLLBAR_X - M1_MENU_HIGHLIGHT_X - 1` to leave a
1px gap before the scrollbar.  Draw a 3px-wide track at `M1_MENU_SCROLLBAR_X`
(124) spanning the menu area, with a filled handle whose Y position is
proportional to the selected item index.  Never hardcode x = 0 or width
= 128 for selection highlights — always use the `M1_MENU_HIGHLIGHT_X` and
`M1_MENU_TEXT_W` constants.
...
| `M1_MENU_HIGHLIGHT_X` | 1 | Highlight left x-inset (1px from left edge) |
| `M1_MENU_TEXT_W` | 122 | Highlight/text width — derived as `M1_MENU_SCROLLBAR_X − M1_MENU_HIGHLIGHT_X − 1` |
...
5. **Highlight box starts at x = `M1_MENU_HIGHLIGHT_X` (1)** with width
   `M1_MENU_TEXT_W` (122px) — never hardcode x = 0 or width = 128.
