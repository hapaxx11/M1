...
**Pure-logic modules (all zero HAL/RTOS/FatFS deps; all covered by host tests):**

| Module | Key exports |
|--------|-------------|
| `Sub_Ghz/subghz_key_encoder.c/h` | KEY→RAW OOK PWM encoding |
| `Sub_Ghz/subghz_raw_line_parser.c/h` | RAW_Data line parsing |
| `Sub_Ghz/subghz_raw_decoder.c/h` | Offline RAW→protocol decode |
| `Sub_Ghz/subghz_playlist_parser.c/h` | Flipper→M1 path remapping |
| `m1_csrc/wifi_ap_record.c/h` | AP record parser, BSSID fmt/parse, MAC check, CSV quote |
| `m1_csrc/wifi_mac_utils.c/h` | MAC address zero-check, format, match |
| `m1_csrc/wifi_file_utils.c/h` | WiFi file extension validators |
| `m1_csrc/wifi_status_msg.c/h` | Pollable timed status message model |
| `m1_csrc/wifi_sta_record.h` | `wifi_sta_t` struct typedef (header-only) |
| `m1_csrc/wifi_selection.c/h` | AP/STA selection counters |
| `m1_csrc/wifi_deauth_cmd.c/h` | Deauth multi-target command builder |
| `m1_csrc/nfc_card_info.c/h` | ISO 7816-6 manufacturer lookup, SAK/ATQA classifier, UID fmt |
| `m1_csrc/nfc_ndef_parse.c/h` | NDEF TLV record parser (`ndef_parse_records`) |
| `m1_csrc/nfc_file_parse.c/h` | NFC file body parser via `nfc_line_reader_t` vtable |
| `m1_csrc/mfc_layout.c/h` | Mifare Classic sector/block layout helpers |
| `m1_csrc/nfc_classify.c/h` | NFC-A device family classifier |
| `m1_csrc/nfc_ntag.c/h` | NTAG page-count, Amiibo pwd derivation |
| `m1_csrc/esp32_feature_map.c/h` | ESP32 feature→capability-bit classifier, `esp32_firmware_is_sin360()` |
| `m1_csrc/ir_signal_record.c/h` | Protocol name→IRMP ID, path helpers, `ir_block_reader_t` vtable, `ir_cmd_parse`, hex/int32 parsers |
| `m1_csrc/ir_button_map.c/h` | IR button→command bidirectional substring matcher |
| `m1_csrc/flipper_ir.c/h` | `flipper_ir_parse_block()` via `ir_block_reader_t` vtable |
| `m1_csrc/badusb_parser.c/h` | DuckyScript line classifier, HID key/modifier tables |

**Shared submenu widget:** `m1_csrc/m1_submenu.c/h` — `m1_submenu_event()` +
`m1_submenu_draw()`.  All simple-label-list menus use `subghz_submenu_model_t` +
this widget.
