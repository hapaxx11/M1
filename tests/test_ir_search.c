 ...
 * test_ir_search.c
 *
 * Unit tests for ir_str_contains_icase() — case-insensitive substring matcher
...
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("Samsung_TV.ir", "Samsung_TV.ir"));
}

void test_substring_at_start(void)
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("Samsung_TV.ir", "Sam"));
}

void test_substring_in_middle(void)
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("Samsung_TV.ir", "sung"));
}

void test_substring_at_end(void)
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("Samsung_TV.ir", ".ir"));
}

void test_no_match(void)
{
	TEST_ASSERT_FALSE(ir_str_contains_icase("Samsung_TV.ir", "Sony"));
}

void test_needle_longer_than_haystack(void)
{
	TEST_ASSERT_FALSE(ir_str_contains_icase("TV.ir", "Samsung_TV.ir"));
}
...
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("Pioneer_AV.ir", "pIoNeEr"));
}
...
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("Samsung_TV.ir", ""));
}

void test_null_needle_matches_everything(void)
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("Samsung_TV.ir", NULL));
}

void test_null_haystack_no_match(void)
{
	TEST_ASSERT_FALSE(ir_str_contains_icase(NULL, "Samsung"));
}
...
void test_null_haystack_empty_needle_matches(void)
{
	...
	TEST_ASSERT_TRUE(ir_str_contains_icase(NULL, ""));
}

void test_single_char_match(void)
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("a", "a"));
}

void test_single_char_no_match(void)
{
	TEST_ASSERT_FALSE(ir_str_contains_icase("a", "b"));
}

void test_needle_equals_haystack_length(void)
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("abc", "abc"));
	TEST_ASSERT_FALSE(ir_str_contains_icase("abc", "xyz"));
}
...
void test_irdb_brand_search(void)
{
	...
	TEST_ASSERT_TRUE(ir_str_contains_icase("Sony_KD75.ir", "kd75"));
	TEST_ASSERT_FALSE(ir_str_contains_icase("Sony_KD75.ir", "samsung"));
}

void test_irdb_partial_model(void)
{
	TEST_ASSERT_TRUE(ir_str_contains_icase("Panasonic_TX55.ir", "tx"));
	TEST_ASSERT_TRUE(ir_str_contains_icase("Panasonic_TX55.ir", "55"));
}
...
void test_irdb_extension_search(void)
{
	...
	TEST_ASSERT_TRUE(ir_str_contains_icase("Samsung_TV.ir", ".ir"));
	TEST_ASSERT_TRUE(ir_str_contains_icase("LG_Soundbar.ir", ".ir"));
}
