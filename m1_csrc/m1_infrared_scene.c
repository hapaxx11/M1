...
static subghz_submenu_model_t s_ir_menu_model;

static void menu_on_enter(M1SceneApp *app)
{
    (void)app;
    if (s_ir_menu_model.item_count == 0)
        subghz_submenu_model_init(&s_ir_menu_model, MENU_ITEM_COUNT,
...
{
    return m1_submenu_event(app, event, &s_ir_menu_model, menu_targets);
}
...
{
    (void)app;
    m1_submenu_draw(&s_ir_menu_model, "Infrared", menu_labels);
}
