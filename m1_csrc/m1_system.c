...
uint8_t                 m1_led_lowbatt_r = (uint8_t)~0x33;  /* ~#331480 default */
uint8_t                 m1_led_lowbatt_g = (uint8_t)~0x14;  /* ~#331480 default */
uint8_t                 m1_led_lowbatt_b = (uint8_t)~0x80;  /* ~#331480 default */
...
void startup_config_handler(void)
{
	...
	M1_LOG_I(M1_LOGDB_TAG, "Device firmware version %d.%d.%d.%d-Hapax.%d\r\n", m1_device_stat.config.fw_version_major,
			m1_device_stat.config.fw_version_minor, m1_device_stat.config.fw_version_build, m1_device_stat.config.fw_version_rc, M1_HAPAX_REVISION);
} // void startup_config_handler(void)
...
/*============================================================================*/
/*
 * Draw the battery indicator (frame + proportional fill + charging bolt +
 * percentage text) at the top-right corner of the display.
 *
 * The actual drawing is shared with the main menu via
 * m1_draw_battery_indicator() (m1_display.c) so both screens show an
 * identical battery glyph.
 */
/*============================================================================*/
static void splash_draw_battery_indicator(void)
{
	...
	m1_draw_battery_indicator(M1_LCD_DISPLAY_WIDTH - 2, 2);
}
