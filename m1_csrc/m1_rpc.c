 ...
 * init=0xFFFF, no reflect, no final XOR).
 *
 * Fixed: the previous production table diverged from the standard table
 * in 46 entries (indices 0x40-0x5F and 0xD7-0xE4). That corrupted table
 * caused connected clients (e.g. qMonstatek) to fall back to a "legacy /
 * compatibility mode" CRC dialect, and larger multi-byte RPC payloads
 * (which are far more likely to touch the diverging entries than short
 * commands) would fail CRC validation intermittently — most visibly on
 * WiFi feature commands, which carry larger variable-length payloads
 * (scan/AP records, connect parameters) than simple commands like PING.
 */
