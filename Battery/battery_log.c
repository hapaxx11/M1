...
static FRESULT battery_log_WriteHeader(FIL *fp, const S_M1_Power_Status_t *log)
{
    ...
    //return f_write(fp, header, strlen(header), &bw);
}
...
extern BQ27421_DataBlock goldenImage_data[14];
bool ExtractGoldenImage(void)
{
	WriteGoldenImage("0:Golden_Image.txt", goldenImage_data, 14);
	return true;
}
